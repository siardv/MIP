xlsx_to_syntax <- function(path, id, answer, value_label, user_na, save_to){
  `%!in%` <- Negate(`%in%`)
  ...
  sapply(seq_along(path), function(i) {
    ...
    c(paste0("/* syntax for ", gsub("^.*/", "~/", path[i]), " (n = ", nrow(df), ").\n\n"),
      ...
      paste0(gsub(", ", "\n", toString(paste0(value_label[[1]], " '", value_label[[2]], "'"))), ".\nEXECUTE.\n"),
      unlist(sapply(1:nrow(df), function(r) unlist(ifelse(r == nrow(df), 
                                                          list(paste0(get_syntax(df, r, id, answer, value_label, user_na), 
                                                          ...
                                                          list(get_syntax(df, r, id, answer, value_label, user_na)))))))
